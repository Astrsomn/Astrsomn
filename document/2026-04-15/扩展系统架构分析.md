...
- `extensionCode`：模型场景优先使用，缺失时会回退到 `extensionKey`。
...
- `ExtensionJarMetadataReader`
  - 输入：待解析 jar 文件。
  - 输出：`SystemExtensionMetaData`（extensionKey/name/type/extensionCode 等）。
...
- `ExtensionCapabilityResolver`
  - 输入：扩展实体（extensionCode/extensionKey）。
...
1. 提供 SPI 实现（Maven 路径或插件 JAR 路径）：
   ...
   - vector：`VecDriver`
2. 保证 `extensionKey/extensionCode` 命名与数据库 `provider` 对齐。
