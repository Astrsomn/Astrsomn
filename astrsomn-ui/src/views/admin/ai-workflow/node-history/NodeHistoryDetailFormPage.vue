    ...
    <div class="form-page">
      ...
      <a-form class="detail-form" layout="vertical" :model="form">
        <a-row :gutter="16">
          ...
          <a-col :xs="24" :md="12"><a-form-item label="executionMs"><a-input v-model:value="form.executionMs" disabled /></a-form-item></a-col>
...
const applyForm = (detail: WorkflowRuntimeRecord) => {
  ...
  form.executionMs = String(detail.executionMs ?? '')
...
const goBack = () => {
  void router.push({ name: 'AdminWorkflowNodeHistory' })
}

const loadDetail = async (id: string) => {
  try {
    const detail = await aiWorkflowRuntimeApi.nodeHistoryDetail(id)
    applyForm(detail)
  } catch (e: unknown) {
