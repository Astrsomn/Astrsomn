...
public class VolcengineProviderHandler extends AbstractModelProviderHandler {
    ...
    private static void applyChatSetting(OpenAiChatModel.OpenAiChatModelBuilder builder, AstroChatParam<?> param) {
        ...
        if (cs.getTemperature() != null
                && VolcengineModelEnum.isParamAvailable(modelKey, AiModelParamEnum.ChatParamEnum.TEMPERATURE.getCode())) {
            builder.temperature(cs.getTemperature());
        }
        if (cs.getTopP() != null
                && VolcengineModelEnum.isParamAvailable(modelKey, AiModelParamEnum.ChatParamEnum.TOP_P.getCode())) {
            builder.topP(cs.getTopP());
        }
        if (cs.getMaxTokens() != null
                && VolcengineModelEnum.isParamAvailable(modelKey, AiModelParamEnum.ChatParamEnum.MAX_TOKENS.getCode())) {
    ...
    public List<AiModelEntity> getAvailableModels() {
        return Arrays.stream(VolcengineModelEnum.values())
                .map(model -> model.toEntity(AiModelEnum.ProviderEnum.VOLCENGINE.getCode()))
                .toList();
    }
