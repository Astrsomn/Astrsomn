...
public class AnthropicProviderHandler extends AbstractModelProviderHandler {
    ...
    public AiModelEnum.ProviderEnum getProvider() {
        return AiModelEnum.ProviderEnum.ANTHROPIC;
    }
    ...
    public <T> T createModel(Class<T> modelClass, AstroChatParam<?> param) {
        ...
        if (StreamingChatModel.class.isAssignableFrom(modelClass)) {
            ...
            throw new UnknowModelException("Failed to initialize: " + modelClass.getName() + " is not supported by Anthropic provider.");
        }
