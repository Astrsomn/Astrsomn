...
public class TencentProviderHandler extends AbstractModelProviderHandler {
    ...
    private static void applyChatSetting(OpenAiChatModel.OpenAiChatModelBuilder builder, AstroChatParam<?> param) {
        ...
        if (cs.getTemperature() != null
                && TencentModelEnum.isParamAvailable(modelKey, AiModelParamEnum.ChatParamEnum.TEMPERATURE.getCode())) {
            builder.temperature(cs.getTemperature());
        }
        if (cs.getTopP() != null
                && TencentModelEnum.isParamAvailable(modelKey, AiModelParamEnum.ChatParamEnum.TOP_P.getCode())) {
            builder.topP(cs.getTopP());
        }
        if (cs.getMaxTokens() != null
                && TencentModelEnum.isParamAvailable(modelKey, AiModelParamEnum.ChatParamEnum.MAX_TOKENS.getCode())) {
    ...
    public AiModelEnum.ProviderEnum getProvider() {
        return AiModelEnum.ProviderEnum.TENCENT;
    }
    ...
    public <T> T createModel(Class<T> modelClass, AstroChatParam<?> param) {
        ...
        if (StreamingChatModel.class.isAssignableFrom(modelClass)) {
            ...
            throw new UnknowModelException("Failed to initialize: " + modelClass.getName() + " is not supported by Tencent provider.");
        }
    ...
    public List<AiModelEntity> getAvailableModels() {
        return Arrays.stream(TencentModelEnum.values())
                .map(model -> model.toEntity(AiModelEnum.ProviderEnum.TENCENT.getCode()))
                .toList();
    }
